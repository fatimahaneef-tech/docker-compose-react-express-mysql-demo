...
http://localhost:3000
in your web browser.
The backend service container has port 80 mapped to port 80 on the host. Test the backend with:
bash
curl localhost:80
Example response:
plain
...
Stop and remove containers and the network	docker compose down
Stop and remove containers, network, and named volumes	docker compose down -v
